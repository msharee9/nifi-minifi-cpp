...
void handle_heartbeat_post_request(coap_context_t * ctx, struct coap_resource_t * resource, coap_session_t * session, coap_pdu_t * pdu,
    ...
    coap_block_t block1;
    printf("Handle heartbeat request\n");
    ...
    if (coap_get_block(pdu, COAP_OPTION_BLOCK1, &block1)) {
        ...
        coap_string_t * payld = (coap_string_t *)ctx->app;
    ...
    if (!heartbeat) {
        printf("heartbeat extraction failed\n");
        return;
    }
...
int main(int argc, char ** argv) {
    ...
    if (argc < 3) {
        printf("Usage:./c2_server <listen ip> <port> <c2 ssl conf>\n");
        return 0;
    }

    properties_t * dtls_props = NULL;
    if (argc >= 4) {
        dtls_props = read_configuration_file(argv[3]);
    }

    dtls_config dc;
    memset(&dc, 0, sizeof(dtls_config));

    if (validate_ssl_configuration(dtls_props, &dc) < 0) {
        printf("Failed to create dtls configuration\n");
    }

    setup_c2_signal_action();
    little_endian = is_little_endian();
    CoapServerContext * coapctx = create_secure_server(argv[1], argv[2], &dc);
    if (!coapctx) {
        printf("Could not create c2 server\n");
        free_properties(dtls_props);
        free_dtls_config(dc);
        return 1;
    }
