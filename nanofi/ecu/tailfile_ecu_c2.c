...
int main(int argc, char ** argv) {
    ...
    if (argc < 2) {
        printf("usage ./tailfile_ecu_c2 <c2 conf file>\n");
        return 1;
    }
    set_log_level(trace);
    setup_ecu_signal_action();

    ecu_context_t * ecu = create_ecu("tailfile_s2s", TAILFILE, SITE2SITE);
    if (!ecu) {
        logc(err, "%s", "ecu creation failed");
        return 1;
    }

    properties_t * c2_props = read_configuration_file(argv[1]);
    if (!c2_props) {
        logc(err, "%s", "Failed to get c2 configuration\n");
    }

    c2context_t * c2 = create_c2_agent(c2_props);

    if (!c2) {
        free_properties(c2_props);
        logc(err, "%s", "Faile to create c2 agent\n");
    }
