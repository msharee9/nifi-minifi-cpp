...
void insert_coap_message(c2context_t * c2, const struct coap_context_t * ctx, const struct coap_message * message) {
    ...
    cm->coap_msg.data = (uint8_t *)malloc(message->length * sizeof(uint8_t));
...
void receive_error(void * receiver_context, coap_context_t * ctx, unsigned int code) {
    //propagate error to c2 context for logging
}
