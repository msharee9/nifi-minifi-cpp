...
c2heartbeat_t prepare_c2_heartbeat(c2context_t * ctx, uint64_t start_time) {
    assert(ctx);

    c2heartbeat_t c2_heartbeat;
    memset(&c2_heartbeat, 0, sizeof(c2heartbeat_t));

    //strcpy(c2_heartbeat.agent_info.ident, ctx->config.agent_id);
    copystr(ctx->config.agent_id, &c2_heartbeat.agent_info.ident);
    c2_heartbeat.agent_info.uptime = time(NULL) - start_time;
    copystr(ctx->config.agent_class, &c2_heartbeat.agent_info.agent_class);
